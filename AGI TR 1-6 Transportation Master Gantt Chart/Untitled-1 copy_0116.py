...
def load_tasks_from_tsv(tsv_path, project_start_date):
    ...
    for headers, row in iter_rows_preserve_headers(tsv_path):
        ...
        for part in reversed(id_parts):
            ...
            if candidate.upper().startswith('A') and candidate[1:].isdigit():
...
def create_comparison_summary(
    ...
    comparison_data = [
        (f"Total Tasks", f"=COUNTIF('{sched_a}'!C:C,\"A*\")", f"=COUNTIF('{sched_b}'!C:C,\"A*\")", "=C4-B4", "Task count difference"),
        (f"Total Voyages", f"=COUNTIF('{sched_a}'!A:A,\"V*\")", f"=COUNTIF('{sched_b}'!A:A,\"V*\")", "=C5-B5", ""),
        (f"Project Duration (days)", f"=MAX('{sched_a}'!H:H)-PROJECT_START", f"=MAX('{sched_b}'!H:H)-PROJECT_START", "=C6-B6", "Days difference"),
        (f"Total Jack-down Events", f"=COUNTIF('{sched_a}'!E:E,\"JACKDOWN\")", f"=COUNTIF('{sched_b}'!E:E,\"JACKDOWN\")", "=C7-B7", ""),
        (f"Project End Date", f"=MAX('{sched_a}'!H:H)", f"=MAX('{sched_b}'!H:H)", "=C8-B8", "Date difference"),
    ]
...
def create_option_schedule_comparison_sheet(
    ...
    id_match_col = "C"
    ids_a = collect_ids(ws_a, 3)
    ids_b = collect_ids(ws_b, 3)
    if not ids_a and not ids_b:
        ids_a = collect_ids(ws_a, 1)
        ids_b = collect_ids(ws_b, 1)
        id_match_col = "A"
    ...
    headers = [
        "Activity ID",
        f"Activity Name ({short_a})",
        f"Phase ({short_a})",
        f"Start ({short_a})",
        f"End ({short_a})",
        f"Dur ({short_a})",
        f"Activity Name ({short_b})",
        f"Phase ({short_b})",
        f"Start ({short_b})",
        f"End ({short_b})",
        f"Dur ({short_b})",
    ...
    def idxmatch(row_idx: int, sheet: str, col_letter: str) -> str:
        # =IFERROR(INDEX('Sheet'!D:D, MATCH($A8, 'Sheet'!$C:$C, 0)), "")
        return f"""=IFERROR(INDEX('{sheet}'!{col_letter}:{col_letter},MATCH($A{row_idx},'{sheet}'!${id_match_col}:${id_match_col},0)),"")"""
    ...
    for n, task_id in enumerate(all_ids):
        ...
        ws_cmp.cell(r, 2, value=idxmatch(r, sched_a, "D"))
        ws_cmp.cell(r, 3, value=idxmatch(r, sched_a, "E"))
        ws_cmp.cell(r, 4, value=idxmatch(r, sched_a, "G"))
        ws_cmp.cell(r, 5, value=idxmatch(r, sched_a, "H"))
        ws_cmp.cell(r, 6, value=idxmatch(r, sched_a, "I"))

        ws_cmp.cell(r, 7, value=idxmatch(r, sched_b, "D"))
        ws_cmp.cell(r, 8, value=idxmatch(r, sched_b, "E"))
        ws_cmp.cell(r, 9, value=idxmatch(r, sched_b, "G"))
        ws_cmp.cell(r, 10, value=idxmatch(r, sched_b, "H"))
        ws_cmp.cell(r, 11, value=idxmatch(r, sched_b, "I"))
...
def create_gantt_with_vba(
    ...
    summary_data.extend([
        ("Project Complete", "=MAX(Schedule_Data_Option_A!H:H,Schedule_Data_Option_B!H:H)"),
        ("", ""),
        ("Weather Constraints", ""),
