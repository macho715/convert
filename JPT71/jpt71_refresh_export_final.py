# Py3.11.8
"""
JPT71 - Refresh + Export FINAL sheets (values only for external sending)

What it does
1) Rebuild Cross_Gantt from Plan (like jpt71_refresh_all.py) into a refreshed workbook (OUT_REFRESHED.xlsx)
2) Using Excel COM (Office installed), open refreshed workbook, recalc, then create FINAL sheets
   inside the same workbook (no new file):
   - Shipping_List_FINAL: copied as VALUES + FORMATS (+ column widths)
   - Mail_Draft_FINAL:    copied as VALUES + FORMATS (+ column widths)
   - Cross_Gantt_FINAL:   copied as VALUES + FORMATS (+ column widths)  (snapshot)

Usage:
  py jpt71_refresh_export_final.py "IN.xlsm" "OUT_REFRESHED.xlsx"
...
def main(in_path: str, out_refreshed: str):
    ...
    print("Added FINAL sheets in:", out_refreshed)
