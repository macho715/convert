...
def load_tasks_from_tsv(tsv_path, project_start_date):
    ...
    def infer_phase_from_task(task_name):
        ...
        if "MOBILIZATION" in task_upper or "DEMOBILIZATION" in task_upper or "DEMOB" in task_upper:
            return "MOBILIZATION"
        elif "DECK" in task_upper and ("PREP" in task_upper or "PREPARATION" in task_upper):
            return "DECK_PREP"
        elif "LOADOUT" in task_upper or "LOAD-OUT" in task_upper or "LOAD IN" in task_upper or "LOAD-IN" in task_upper:
            return "LOADOUT"
        elif "SEA FASTENING" in task_upper or "SEAFAST" in task_upper or "SEAFASTENING" in task_upper:
            return "SEAFAST"
        elif "SAIL" in task_upper or "SAIL-AWAY" in task_upper:
            return "SAIL"
        elif "UNLOAD" in task_upper or "ARRIVAL" in task_upper or "LOAD-IN" in task_upper:
            return "AGI_UNLOAD"
        elif "TURNING" in task_upper or "TURN" in task_upper:
            return "TURNING"
        elif "JACKDOWN" in task_upper or "JACK-DOWN" in task_upper or "JACKING DOWN" in task_upper:
            return "JACKDOWN"
        ...
        try:
            ...
            if dur_val == 0:
                ...
                if "Loadout" in task_name or "Load-out" in task_name:
                    ...
                    return "DUR_SAIL"
                elif "Arrive" in task_name or "Unload" in task_name or "Load-in" in task_name:
                    return "DUR_UL"
    ...
    with open(tsv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # Task 확인
            task_name = (row.get('Activity Name', '').strip() or row.get('Task', '').strip())
            if not task_name:
                continue
...
def create_comparison_summary(
    ...
    comparison_data = [
        (f"Total Tasks", f"=COUNTA(Schedule_Data_{scenario_a}!A:A)-5", f"=COUNTA(Schedule_Data_{scenario_b}!A:A)-5", "=C4-B4", "Task count difference"),
        (f"Total Voyages", f"=COUNTIF(Schedule_Data_{scenario_a}!A:A,\"V*\")", f"=COUNTIF(Schedule_Data_{scenario_b}!A:A,\"V*\")", "=C5-B5", ""),
        (f"Project Duration (days)", f"=MAX(Schedule_Data_{scenario_a}!G:G)-PROJECT_START", f"=MAX(Schedule_Data_{scenario_b}!G:G)-PROJECT_START", "=C6-B6", "Days difference"),
        (f"Total Jack-down Events", f"=COUNTIF(Schedule_Data_{scenario_a}!D:D,\"JACKDOWN\")", f"=COUNTIF(Schedule_Data_{scenario_b}!D:D,\"JACKDOWN\")", "=C7-B7", ""),
        (f"Project End Date", f"=MAX(Schedule_Data_{scenario_a}!G:G)", f"=MAX(Schedule_Data_{scenario_b}!G:G)", "=C8-B8", "Date difference"),
    ]
...
def create_gantt_with_vba(
    ...
    start_tsv = None
    for candidate in [scenario_a_tsv, scenario_b_tsv, scenario_c_tsv, scenario_d_tsv, mobilization_tsv, tsv_path]:
    ...
    # === Scenario Sheets ===
    scenario_a_created = False
    scenario_b_created = False
    scenario_c_created = False
    scenario_d_created = False
    mobilization_created = False

    if scenario_a_tsv and os.path.exists(scenario_a_tsv):
        scenario_a_created = create_scenario_sheets(
            wb,
            "ScenarioA",
            scenario_a_tsv,
            project_start,
            pattern_str="1x1x1x1x1x1x1",
        )
        if scenario_a_created:
            print("Created ScenarioA sheets from TSV")
        else:
            print("Warning: ScenarioA sheets not created from TSV")
    else:
        scenario_a_created = create_scenario_sheets(
            wb,
            "ScenarioA",
        ...
        if scenario_a_created:
            print("Created ScenarioA sheets (auto-generated)")
        else:
            print("Warning: ScenarioA sheets not created (auto-generated)")

    if scenario_b_tsv and os.path.exists(scenario_b_tsv):
        scenario_b_created = create_scenario_sheets(
            wb,
            "ScenarioB",
            scenario_b_tsv,
            project_start,
        ...
        if scenario_b_created:
            print("Created ScenarioB sheets from TSV")
        else:
            print("Warning: ScenarioB sheets not created from TSV")
    else:
        scenario_b_created = create_scenario_sheets(
            wb,
            "ScenarioB",
        ...
        if scenario_b_created:
            print("Created ScenarioB sheets (auto-generated)")
        else:
            print("Warning: ScenarioB sheets not created (auto-generated)")

    if scenario_c_tsv and os.path.exists(scenario_c_tsv):
        scenario_c_created = create_scenario_sheets(
            wb,
            "ScenarioC",
            scenario_c_tsv,
            project_start,
        ...
        if scenario_c_created:
            print("Created ScenarioC sheets from TSV")
        else:
            print("Warning: ScenarioC sheets not created from TSV")
    else:
        scenario_c_created = create_scenario_sheets(
            wb,
            "ScenarioC",
        ...
        if scenario_c_created:
            print("Created ScenarioC sheets (auto-generated)")
        else:
            print("Warning: ScenarioC sheets not created (auto-generated)")

    if scenario_d_tsv and os.path.exists(scenario_d_tsv):
        scenario_d_created = create_scenario_sheets(
            wb,
            "ScenarioD",
            scenario_d_tsv,
            project_start,
        ...
        if scenario_d_created:
            print("Created ScenarioD sheets from TSV")
        else:
            print("Warning: ScenarioD sheets not created from TSV")
    else:
        scenario_d_created = create_scenario_sheets(
            wb,
            "ScenarioD",
        ...
        if scenario_d_created:
            print("Created ScenarioD sheets (auto-generated)")
        else:
            print("Warning: ScenarioD sheets not created (auto-generated)")

    if mobilization_tsv and os.path.exists(mobilization_tsv):
        mobilization_created = create_scenario_sheets(
            wb,
            "Mammoet_Original",
            mobilization_tsv,
            project_start,
    ...
    # === Scenario Comparison ===
    if scenario_a_created and scenario_b_created:
        create_comparison_summary(
            wb,
            project_start,
            scenario_a="ScenarioA",
            scenario_b="ScenarioB",
            label_a="ScenarioA (1x1x1x1x1x1x1)",
            label_b="ScenarioB (1-2-2-2)",
            sheet_name="Scenario_Comparison",
        )
        print("Created Scenario_Comparison sheet")

    if scenario_c_created and scenario_d_created:
        create_comparison_summary(
            wb,
            project_start,
            scenario_a="ScenarioC",
            scenario_b="ScenarioD",
            label_a="ScenarioC (2-2-2-1 Single)",
            label_b="ScenarioD (2-2-2-1 Two SPMTs)",
            sheet_name="Scenario_Comparison_2221",
        )
        print("Created Scenario_Comparison_2221 sheet")
...
Sub UpdateAllScenarios()
    ...
    Sheets("Schedule_Data_ScenarioC").Calculate
    Sheets("Gantt_Chart_ScenarioC").Calculate
    Sheets("Schedule_Data_ScenarioD").Calculate
    Sheets("Gantt_Chart_ScenarioD").Calculate
    Sheets("Schedule_Data_Mammoet_Original").Calculate
    Sheets("Gantt_Chart_Mammoet_Original").Calculate
    Sheets("Tide_Data").Calculate
    Sheets("Scenario_Comparison").Calculate
    Sheets("Scenario_Comparison_2221").Calculate
    On Error GoTo 0
...
Sub RefreshAllGanttCharts()
    ...
    Call RefreshGanttChart_ScenarioC
    Call RefreshGanttChart_ScenarioD
...
' === ScenarioC Gantt 갱신 ===
Sub RefreshGanttChart_ScenarioC()
    Dim ws As Worksheet, wsd As Worksheet
    Dim i As Long, j As Long, lastRow As Long, ganttRow As Long
    Dim startD As Date, endD As Date, projStart As Date, cellDate As Date
    Dim phase As String, dc As Long, lastCol As Long, maxJ As Long
    ...
    Set ws = Sheets("Gantt_Chart_ScenarioC")
    Set wsd = Sheets("Schedule_Data_ScenarioC")
    projStart = Sheets("Control_Panel").Range("B4").Value
    shamalStart = Sheets("Control_Panel").Range("H5").Value
    shamalEnd = Sheets("Control_Panel").Range("H6").Value
    dc = 8
...
' === ScenarioD Gantt 갱신 ===
Sub RefreshGanttChart_ScenarioD()
    Dim ws As Worksheet, wsd As Worksheet
    Dim i As Long, j As Long, lastRow As Long, ganttRow As Long
    Dim startD As Date, endD As Date, projStart As Date, cellDate As Date
    Dim phase As String, dc As Long, lastCol As Long, maxJ As Long
    ...
    Set ws = Sheets("Gantt_Chart_ScenarioD")
    Set wsd = Sheets("Schedule_Data_ScenarioD")
    projStart = Sheets("Control_Panel").Range("B4").Value
    shamalStart = Sheets("Control_Panel").Range("H5").Value
    shamalEnd = Sheets("Control_Panel").Range("H6").Value
    dc = 8
...
Sub RefreshGanttChart_Mammoet_Original()
    ...
    Set ws = Sheets("Gantt_Chart_Mammoet_Original")
    Set wsd = Sheets("Schedule_Data_Mammoet_Original")
    projStart = Sheets("Control_Panel").Range("B4").Value
    shamalStart = Sheets("Control_Panel").Range("H5").Value
    shamalEnd = Sheets("Control_Panel").Range("H6").Value
    dc = 8
...
if __name__ == "__main__":
    ...
    scenario_b_tsv_primary = os.path.join(script_dir, "ScenarioB_1-2-2-2_start2026-01-15.tsv")
    scenario_b_tsv_alt = os.path.join(script_dir, "Original_1-2-2-2_Single SPMT.tsv")
    scenario_b_tsv = scenario_b_tsv_primary if os.path.exists(scenario_b_tsv_primary) else scenario_b_tsv_alt
    scenario_c_tsv = os.path.join(script_dir, "Original_2-2-2-1_Single SPMT.tsv")
    scenario_d_tsv = os.path.join(script_dir, "Alternative_2-2-2-1_Two SPMTs.tsv")
    ...
    wb = create_gantt_with_vba(
        ...
        scenario_c_tsv=scenario_c_tsv if os.path.exists(scenario_c_tsv) else None,
        scenario_d_tsv=scenario_d_tsv if os.path.exists(scenario_d_tsv) else None,
