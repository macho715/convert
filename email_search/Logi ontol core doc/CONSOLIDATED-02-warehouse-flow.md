...
- **Site Arrival Date (v3.5 Extension)**: siteArrivalDate (generic), shuArrivalDate, mirArrivalDate, dasArrivalDate, agiArrivalDate (site-specific, subPropertyOf siteArrivalDate)
...
- __Rule-8B__: Flow Code 5는 MOSB 있으나 Site 없음 또는 WH 2개 이상 + MOSB 없음 조건으로 분류.
  - **Site 없음 판정**: `siteArrivalDate` (또는 shuArrivalDate/mirArrivalDate/dasArrivalDate/agiArrivalDate)가 모두 없으면 Site 미도착으로 판정
  - **MOSB 있음 판정**: `debulk:mosbArrivalDate` 또는 MOSB 컬럼에 날짜 존재
