...
def load_tasks_from_tsv(tsv_path, project_start_date):
    ...
    입력 파일이 여러 형태(중복 헤더, Activity ID(1..3), ID/WBS/Phase 등)로 제공되는
    현실을 고려해 다음 규칙으로 파싱합니다.

    - Activity ID가 다단(1~3)로 제공되면: 가장 상세 레벨(보통 3)을 우선 ID로 사용
    - Activity ID 헤더가 중복(Excel export)인 경우: 첫 3개 컬럼을 계층 ID로 취급
    - ID가 비어 있으면: 계층 ID를 결합한 surrogate ID를 생성

    반환 tuple 형식:
        (task_id, wbs, task_name, phase, owner, offset_days, duration_ref, notes)

    NOTE:
        - Start/End는 프로젝트 시작일(PROJECT_START) 기준 offset으로 변환하여
          엑셀에서는 '=PROJECT_START+offset' 형태로 일관되게 계산되도록 합니다.
        - 날짜 문자열은 다양한 포맷을 허용하지만, 가능하면 원본 TSV에서 ISO(YYYY-MM-DD)
          형태를 사용하는 것을 권장합니다.
    """
    ...
    # ------------------------------
    # Duration 매핑 (기존 로직 유지)
    # ------------------------------
    def get_duration_ref(duration_str: str, task_name: str):
        try:
            ...
            if dur_val == 0.5:
                return 'DUR_BUF'
            if dur_val == 1.0:
                if 'LOADOUT' in task_name.upper() or 'LOAD-OUT' in task_name.upper():
                    return 'DUR_LO'
                if 'MOBILIZATION' in task_name.upper() or 'DEMOBILIZATION' in task_name.upper():
                    return 'DUR_MOB'
                if 'SAIL' in task_name.upper():
                    return 'DUR_SAIL'
                if 'ARRIV' in task_name.upper() or 'UNLOAD' in task_name.upper() or 'LOAD-IN' in task_name.upper():
                    return 'DUR_UL'
                if 'RETURN' in task_name.upper():
                    return 'DUR_RET'
                if 'JACK' in task_name.upper():
                    return 'DUR_JD'
                return 'DUR_BUF'
            if dur_val == 2.0:
                return 2
            if dur_val == 3.0:
                if 'TURN' in task_name.upper():
                    return 'DUR_TURN'
                if 'DECK' in task_name.upper() or 'PREP' in task_name.upper():
                    return 'DUR_DECK'
                return 3
            return dur_val
        except Exception:
    ...
    def parse_date(date_str: str):
        ...
        # 12-Jan (연도는 project_start_date에서 추론)
        try:
    ...
    for headers, row in iter_rows_preserve_headers(tsv_path):
        ...
        # --- multi-level activity id columns ---
        id_cols: list[int] = []

        # Option B style: Activity ID (1..3)
        for k in ('Activity ID (1)', 'Activity ID (2)', 'Activity ID (3)'):
            i = idx(k)
            if i is not None:
                id_cols.append(i)

        # Excel export style: duplicated 'Activity ID'
        if not id_cols:
            dup = [i for i, h in enumerate(header_lc) if h == 'Activity ID']
            if dup:
                id_cols = dup[:3]

        # Simple style: single 'Activity ID' or 'ID'
        if not id_cols:
            i = idx('Activity ID')
            if i is not None:
                id_cols = [i]
        ...
        # Determine concrete task id
        task_id = ''
        # prefer something like A1234
        for part in reversed(id_parts):
            if part.upper().startswith('A') and part[1:].isdigit():
                task_id = part.upper()
                break
        if not task_id and id_parts:
        ...
        if name_i is None:
            name_i = idx('Task')
        task_name = row[name_i].strip() if name_i is not None else ''
        if not task_name:
            continue
        ...
        dur_ref = get_duration_ref(duration_str or '0', task_name)
        ...
        # If ID still empty, generate deterministic surrogate
        if not task_id:
            task_counter += 1
            task_id = f'TASK-{task_counter:04d}'

        # WBS: if present use, else create a stable sequence
        wbs_i = idx('WBS')
        if wbs_i is not None and row[wbs_i].strip():
            wbs = row[wbs_i].strip()
        else:
            # keep ordering by file position (float-like text keeps sort stable)
            wbs = f'{len(tasks)+1:.1f}'

        # Offset days from PROJECT_START
...
def create_option_schedule_comparison_sheet(
    ...
    """Option A/B 전체 일정(행 단위)을 비교하는 시트를 생성합니다.

    - 기준 키: ID 컬럼
    - 조회: INDEX/MATCH (Office 2010+ 호환)
    - 시작/종료일은 각 Schedule_Data 시트의 계산 결과를 참조하므로
      Control_Panel의 PROJECT_START 변경 시 자동 갱신됩니다.
    """
    ...
    ids_a = collect_ids(ws_a)
    ids_b = collect_ids(ws_b)

    # Option A order first; then append IDs unique to Option B
    seen = set()
    all_ids = []
    for x in ids_a + ids_b:
        if x in seen:
            continue
        seen.add(x)
        all_ids.append(x)
    ...
    ws_cmp["A1"] = f"{short_a} vs {short_b} - Full Schedule Comparison"
    ...
    ws_cmp["B3"] = f"=MAX('{sched_a}'!G:G)"
    ws_cmp["A4"] = "Overall End (B)"
    ws_cmp["B4"] = f"=MAX('{sched_b}'!G:G)"
    ...
    def idxmatch(row_idx: int, sheet: str, col_letter: str) -> str:
        # =IFERROR(INDEX('Sheet'!C:C, MATCH($A8, 'Sheet'!$A:$A, 0)), "")
        return f"""=IFERROR(INDEX('{sheet}'!{col_letter}:{col_letter},MATCH($A{row_idx},'{sheet}'!$A:$A,0)),"")"""


    start_row = 8
    for n, task_id in enumerate(all_ids):
        r = start_row + n
        ws_cmp.cell(r, 1, value=task_id)

        ws_cmp.cell(r, 2, value=idxmatch(r, sched_a, "C"))
        ws_cmp.cell(r, 3, value=idxmatch(r, sched_a, "D"))
        ws_cmp.cell(r, 4, value=idxmatch(r, sched_a, "F"))
        ws_cmp.cell(r, 5, value=idxmatch(r, sched_a, "G"))
        ws_cmp.cell(r, 6, value=idxmatch(r, sched_a, "H"))

        ws_cmp.cell(r, 7, value=idxmatch(r, sched_b, "C"))
        ws_cmp.cell(r, 8, value=idxmatch(r, sched_b, "D"))
        ws_cmp.cell(r, 9, value=idxmatch(r, sched_b, "F"))
        ws_cmp.cell(r, 10, value=idxmatch(r, sched_b, "G"))
        ws_cmp.cell(r, 11, value=idxmatch(r, sched_b, "H"))
    ...
    try:
        ...
        last_row = start_row + len(all_ids) - 1
...
def create_gantt_with_vba(
    ...
    start_tsv = None
    for candidate in [option_a_tsv, option_b_tsv, tsv_path]:
    ...
    summary_items = [
        ("A9", "Estimated Completion:", "B9", "=MAX(Schedule_Data_Option_A!G:G,Schedule_Data_Option_B!G:G)"),
    ...
    buttons = [
        "▶ UpdateAllScenarios - Batch update (Option A/B + Compare)",
    ...
    summary_data.extend([
        ("Project Complete", "=MAX(Schedule_Data_Option_A!G:G,Schedule_Data_Option_B!G:G)"),
        ("", ""),
        ("Weather Constraints", ""),
...
Public Sub UpdateAllScenarios()
    ...
    Sheets("Option_Compare").Calculate
...
CleanUp:
    ...
    Else
        MsgBox "✅ Option A/B + Compare 업데이트 완료", vbInformation, "Update Complete"
    End If
End Sub
...
Private Sub RefreshGanttCore(ByVal ganttSheetName As String, ByVal schedSheetName As String)
    ...
    ' Apply Gantt bars
    For i = 6 To lastRow
        If Trim$(CStr(wsd.Cells(i, 1).Value)) = "" Then GoTo NextRow

        startD = CoerceDate(wsd.Cells(i, 6).Value, projStart)
        endD = CoerceDate(wsd.Cells(i, 7).Value, projStart)
        If startD = 0 Then GoTo NextRow
        If endD = 0 Then endD = startD

        phase = CStr(wsd.Cells(i, 4).Value)
...
Public Sub ExportToPDF()
    ...
    Sheets(Array("Control_Panel", "Scenario_Comparison", "Option_Compare", "Gantt_Chart_Option_A", "Gantt_Chart_Option_B")).Select
...
Public Sub GenerateReport()
    ...
    endA = WorksheetFunction.Max(Sheets("Schedule_Data_Option_A").Range("G:G"))
    endB = WorksheetFunction.Max(Sheets("Schedule_Data_Option_B").Range("G:G"))
...
Private Function CheckShamalRiskOne(ByVal schedName As String, ByVal shamalStart As Date, ByVal shamalEnd As Date, ByRef outList As String) As Long
    ...
    For i = 6 To lastRow
        taskDate = CoerceDate(wsd.Cells(i, 6).Value)
        If taskDate <> 0 Then
            If taskDate >= shamalStart And taskDate <= shamalEnd Then
                If wsd.Cells(i, 4).Value = "SAIL" Or wsd.Cells(i, 4).Value = "LOADOUT" Then
                    ...
                    outList = outList & vbCrLf & "  - [" & schedName & "] " & wsd.Cells(i, 1).Value & ": " & wsd.Cells(i, 3).Value
                End If
            End If
        End If
    Next i
...
'     On Error GoTo CleanUp
'     If Intersect(Target, Me.Range("B4,H5:H7,H10:H11")) Is Nothing Then Exit Sub
...
if __name__ == "__main__":
    ...
    output_path = os.path.join(
        os.getcwd(),
        f"AGI_TR_MultiScenario_Master_Gantt_VBA_{timestamp}.xlsm",
    )
    wb.save(output_path)
