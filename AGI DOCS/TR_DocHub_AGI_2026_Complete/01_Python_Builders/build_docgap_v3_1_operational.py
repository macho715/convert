...
# VBA 매크로 코드
VBA_MACROS = """Attribute VB_Name = "modDocGapMacros"
Option Explicit
...
def apply_v3_1(src: str, out: str) -> None:
    ...
    if "Inputs" not in wb.sheetnames:
        ...
        ws.cell(1, 1).value = "Inputs Sheet"
        ws.cell(2, 1).value = "Field"
        ws.cell(2, 2).value = "Value (Auto)"
        ws.cell(3, 1).value = "MZP Arrival"
        ws.cell(4, 1).value = "RoRo Start"
        ws.cell(5, 1).value = "RoRo End"
        ws.cell(6, 1).value = "MZP Departure"
    else:
        ws = wb["Inputs"]
    ...
    def patch_req_sheet(sheet_name: str):
        if sheet_name not in wb.sheetnames:
            # Create minimal structure if sheet doesn't exist
            wsx = wb.create_sheet(sheet_name)
            wsx.cell(1, 1).value = f"{sheet_name} - Document Requirements"
            wsx.cell(2, 1).value = "Doc Code"
            wsx.cell(2, 2).value = "Category"
            wsx.cell(2, 3).value = "Document Name"
            wsx.cell(2, 4).value = "Status"
            wsx.cell(2, 5).value = "Risk"
            wsx.cell(2, 6).value = "Owner"
            wsx.cell(2, 7).value = "Submit To"
            wsx.cell(2, 8).value = "Remarks"
            # Add a few sample rows
            for r in range(3, 6):
                wsx.cell(r, 1).value = f"DOC-{r-2:03d}"
                wsx.cell(r, 2).value = "Sample"
                wsx.cell(r, 3).value = f"Sample Document {r-2}"
        else:
            wsx = wb[sheet_name]
    ...
    if "Executive_Summary" not in wb.sheetnames:
        ...
        ws_exec.cell(1, 1).value = "Executive Summary"
        ws_exec.cell(1, 3).value = "Last updated: (will be updated by DG_StampUpdated macro)"
        ws_exec.cell(3, 1).value = "Document Status Overview"
        ws_exec.cell(4, 1).value = "Total Documents"
        ws_exec.cell(4, 2).value = "=COUNTA(OFCO_Req_1_15!A:A)-1"
        ws_exec.cell(5, 1).value = "Missing"
        ws_exec.cell(5, 2).value = "=COUNTIF(OFCO_Req_1_15!D:D,\"Missing\")"
        ws_exec.cell(6, 1).value = "Submitted"
        ws_exec.cell(6, 2).value = "=COUNTIF(OFCO_Req_1_15!D:D,\"Submitted\")"
        ws_exec.cell(7, 1).value = "Approved"
        ws_exec.cell(7, 2).value = "=COUNTIF(OFCO_Req_1_15!D:D,\"Approved\")"

    # ---------------- VBA_Pasteboard sheet ----------------
    # Create or append to VBA_Pasteboard sheet
    if "VBA_Pasteboard" in wb.sheetnames:
        ws_vba = wb["VBA_Pasteboard"]
    elif "VBA_Module" in wb.sheetnames:
        ws_vba = wb["VBA_Module"]
        ws_vba.title = "VBA_Pasteboard"
    else:
        ws_vba = wb.create_sheet("VBA_Pasteboard")

    row = ws_vba.max_row
    if row == 1 and ws_vba.cell(1, 1).value is None:
        row = 1
    ...
    ws_vba.cell(row, 1).value = f"DocGap v3.1 Operational - {timestamp}"
    row += 1
