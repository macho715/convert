...
def load_tasks_from_tsv(tsv_path, project_start_date):
    ...
    # --- NOTES extraction helpers (Primavera/compact TSV without an explicit Notes column) ---
    # The Primavera export used in this project often encodes operational details inside
    # the Activity Name (e.g., equipment requirements after " - ", or sub-steps separated
    # by semicolons). The legacy 2-2-2-1 schedule template carried these as a dedicated
    # Notes column; the newer MOS/MFC1/MFA2 TSVs may not.
    ...
    def _extract_task_and_notes(raw_task: str):
        """Split a Primavera Activity Name into (task, notes).

        Rules (conservative, in order):
          1) If string contains " - ", treat the right side as a notes tail.
          2) If the remaining left side contains semicolons *outside parentheses*,
             keep the first segment as task and push the remaining segments into notes.

        This is only applied when the TSV row does not already provide a Notes column.
        """
        ...
        def split_semicolon_outside_parens(s: str):
            """Split by ';' while ignoring semicolons inside parentheses."""
            parts = []
            buf = []
            depth = 0
            for ch in s:
        ...
        # Trailing notes separated by " - " (equipment requirements etc.)
        if " - " in task_left:
            left, right = task_left.rsplit(" - ", 1)
            if left.strip() and right.strip():
                task_left = left.strip()
                notes_parts.append(right.strip())

        # Semicolon-separated substeps (outside parentheses)
        if ";" in task_left:
            parts = split_semicolon_outside_parens(task_left)
            if len(parts) >= 2:
                task_left = parts[0]
                notes_parts.insert(0, "; ".join(parts[1:]))

        notes = "; ".join([p for p in notes_parts if p])
        return task_left.strip(), notes.strip()
    ...
    with open(tsv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # Task 확인 (raw label + Notes derivation)
            task_id_candidate = (row.get('Activity ID', '').strip() or row.get('ID', '').strip())
            task_name_raw = (row.get('Activity Name', '').strip() or row.get('Task', '').strip())

            # Primavera summary-row export quirk:
            # "Activity Name" can be shifted and contain a numeric duration like "1.00".
            # In that case, fall back to Activity ID for the task label.
            if task_name_raw and _is_number_like(task_name_raw) and task_id_candidate and not _is_number_like(task_id_candidate):
                task_name_raw = task_id_candidate
            if not task_name_raw and task_id_candidate:
                task_name_raw = task_id_candidate
            if not task_name_raw:
                continue

            # Notes (explicit column or derived from Activity Name)
            notes = row.get('Notes', '').strip() if row.get('Notes') is not None else ''
            task_name = task_name_raw  # default (keep full label)
            if not notes:
                task_name, notes = _extract_task_and_notes(task_name_raw)
            
            # ID 처리 (없으면 자동 생성)
            task_id = task_id_candidate
            if not task_id:
                if "VOYAGE" in task_name_raw.upper():
                    voyage_num += 1
                    task_id = f"V{voyage_num}"
                elif "MOBILIZATION" in task_name_raw.upper():
                    task_id = "MOB-001"
                elif "DECK" in task_name_raw.upper() and "PREP" in task_name_raw.upper():
                    task_id = "PREP-001"
                else:
                    task_counter += 1
                    # Task 이름에서 약자 생성
                    words = task_name_raw.split()
                    if words:
            ...
            # NOTE: phase inference should use the *raw* activity name because the derived notes part
            # may contain keywords (Load-in/Load-out/Return/etc.).
            phase_raw = row.get('Phase', '').strip()
            if phase_raw:
                phase = phase_mapping.get(phase_raw, infer_phase_from_task(task_name_raw))
            else:
                phase = infer_phase_from_task(task_name_raw)
            ...
            dur_ref = get_duration_ref(duration_str, task_name_raw)
            ...
            # Notes handled above (explicit or derived)
...
def create_gantt_with_vba(
    ...
    if mammoet_scenarioa_tsv and os.path.exists(mammoet_scenarioa_tsv):
        ...
        if mammoet_scenarioa_created:
            print("Created Mammoet_ScenarioA sheets from Mammoet format_ScenarioA_1x1x1x1x1x1x1.tsv")
        else:
    ...
    if mammoet_alternative_tsv and os.path.exists(mammoet_alternative_tsv):
        ...
        if mammoet_alternative_created:
            print("Created Mammoet_Alternative sheets from Mammoet format_Alternative_2-2-2-1_Two SPMTs.tsv")
        else:
...
if __name__ == "__main__":
    ...
    mammoet_original_new = os.path.join(script_dir, "MOS.tsv")
    ...
    mammoet_scenarioa_new = os.path.join(script_dir, "MFC1.tsv")
    mammoet_scenarioa_old = os.path.join(script_dir, "Mammoet format_ScenarioA_1x1x1x1x1x1x1.tsv")
    mammoet_scenarioa_tsv = (
    ...
    mammoet_alternative_new = os.path.join(script_dir, "MFA2.tsv")
    mammoet_alternative_old1 = os.path.join(script_dir, "Mammoet format_Alternative_2-2-2-1_Two SPMTs.tsv")
    mammoet_alternative_old2 = os.path.join(script_dir, "Mammoet formatAlternative_2-2-2-1_Two SPMTs.tsv")
    mammoet_alternative_tsv = (
        mammoet_alternative_new
        if os.path.exists(mammoet_alternative_new)
        else mammoet_alternative_old1
    ...
    wb = create_gantt_with_vba(
        tsv_path=tsv_path,
        mammoet_original_tsv=mammoet_original_tsv if os.path.exists(mammoet_original_tsv) else None,
        mammoet_scenarioa_tsv=mammoet_scenarioa_tsv if os.path.exists(mammoet_scenarioa_tsv) else None,
        mammoet_alternative_tsv=mammoet_alternative_tsv if os.path.exists(mammoet_alternative_tsv) else None,
