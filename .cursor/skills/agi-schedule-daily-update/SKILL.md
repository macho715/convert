...
description: AGI TR Unit 1 Schedule HTML의 공지란·Weather & Marine Risk 블록 매일 갱신. 공지는 사용자 제공, 날씨는 웹 검색 후 포맷에 맞춰 입력.
---
...
- AGI TR Unit 1 Schedule HTML의 **공지란(Operational Notice)**을 사용자 제공 내용으로 갱신할 때
...
- `AGI TR 1-6 Transportation Master Gantt Chart/AGI TR Unit 1 Schedule_*.html` (예: `AGI TR Unit 1 Schedule_20260126.html`)

## 입력

| 항목 | 제공자 | 형식 | 예시 |
|------|--------|------|------|
| 공지 날짜 | 사용자 | YYYY-MM-DD | 2026-01-26 |
...
| **2. 해상 날씨** | 사용자 수동 다운로드 | PDF·JPG를 `weather/YYYYMMDD/` 에 두면, 에이전트가 **PDF/JPG 파싱** 후 해상 날씨 문단을 날씨란에 삽입. |

- **해상용 파일 경로**: `AGI TR 1-6 Transportation Master Gantt Chart/weather/YYYYMMDD/` (YYYYMMDD = 해당일).
...
**2b) 해상 날씨 (수동 다운로드)**
- **경로**: `AGI TR 1-6 Transportation Master Gantt Chart/weather/YYYYMMDD/` (해당일 폴더). 사용자가 PDF·JPG를 수동 다운로드해 둠.
...
- **JPG OCR 설정**: JPG 파싱은 Tesseract OCR 사용. `eng.traineddata`가 없으면 프로젝트에 `out/tessdata/eng.traineddata`를 두고, `scripts/weather_parse.py`가 자동으로 `TESSDATA_PREFIX=CONVERT/out/tessdata`를 사용. **날씨 파서 확인**: `python scripts/weather_parse.py "AGI TR 1-6 Transportation Master Gantt Chart/weather/YYYYMMDD"` 로 PDF·JPG 파싱 가능 여부 확인.

**2c) PDF 파서 → WEATHER.PY 파이프라인 (4일 히트맵용)**  
- **순서**: (1) PDF/JPG 파싱 → (2) 파서 txt → WEATHER.PY용 JSON 변환 → (3) WEATHER.PY 실행.  
- **1단계**: `python scripts/weather_parse.py "AGI TR 1-6.../weather/YYYYMMDD" --out out/weather_parsed/YYYYMMDD`  
- **2단계**: `python scripts/parsed_to_weather_json.py out/weather_parsed/YYYYMMDD` → `out/weather_parsed/YYYYMMDD/weather_for_weather_py.json` 생성 (ADNOC 텍스트에서 날짜·파고(ft)·풍속(kt) 추출, 30–31일 6 ft 운행 어려움 반영).  
- **3단계**: `AGI TR 1-6...` 폴더에서 `python WEATHER.PY` 실행. 4일 모드일 때 위 JSON이 있으면 자동 사용 → `out/weather_4day_heatmap.png` 생성.  
- 스케줄 HTML Weather 블록에 `out/weather_4day_heatmap.png` 이미지 삽입 시 위 파이프라인 실행 후 생성된 PNG를 사용.
...
- **수정한 날짜(YYYYMMDD)를 파일명 뒤에 넣어 신규 파일로 저장**한다. 원본은 덮어쓰지 않는다.
- 예: `AGI TR Unit 1 Schedule_20260126.html` → 수정일 2026-01-28이면 `AGI TR Unit 1 Schedule_20260128.html` 로 신규 생성.
- 파일명 형식: `AGI TR Unit 1 Schedule_YYYYMMDD.html` (YYYYMMDD = 갱신을 수행한 날짜).

## 안전 규칙

- 공지란·날씨 블록 **밖**의 HTML(간트 데이터, 스크립트, KPI, Voyage Cards 등)은 수정하지 않는다.
- 공지 미제공 시: 해당일로 날짜만 변경하고 기존 내용은 삭제(날짜만 남김). 날씨만 갱신할 때도 공지란에 이 규칙 적용.

## 통합

- Subagent `/agi-schedule-updater`: 이 스킬을 사용해 공지·날씨 블록 갱신 수행.
