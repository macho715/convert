...
'
' Key concept
'   - D0 = Trip-1 Load-out date (Control_Panel!C5)
'   - All tasks use Offset (days from D0) and Duration (days)
'   - UpdateAllDates recalculates Start/End
'   - RefreshGanttChart draws the bars
'   - CheckScheduleConflicts flags Tide + Shamal risks
'
' Sheets expected (auto-created if missing):
'   - Control_Panel
'   - Schedule_Data
'   - Gantt_Chart
'   - Tide_Data
'   - Weather_Analysis (optional)
'   - Summary (optional)
'=====================================================================
...
' === GANTT LAYOUT ===
Private Const GANTT_TITLE_ROW As Long = 1
Private Const GANTT_SUBTITLE_ROW As Long = 2
Private Const GANTT_MONTH_ROW As Long = 3
...
Public Sub BuildScenario_1_2_2_1_JD3()
    ' Writes Scenario (1-2-2-1 voyages, 2 Jack-down batches of 3 units) into Schedule_Data.
    ' Offsets are relative to D0 (Control_Panel!C5).
    ...
    wsData.Range("A3").Value = "Planning notes: Shamal window in Control_Panel; Tide: RoRo requires high tide; use buffers and D0 optimizer"

    ' Clear old data region
    lastRow = GetLastDataRow(wsData)
    If lastRow < DATA_START_ROW Then lastRow = DATA_START_ROW

    wsData.Range(wsData.Cells(DATA_START_ROW, COL_ID), wsData.Cells(lastRow + 100, COL_STATUS)).ClearContents

    ' Task table: (ID, WBS, Task, Phase, Owner, Offset, Duration, Notes, Status)
    Dim T As Variant
    T = Array( _
        Array("MOB-001", "1.0", "MOBILIZATION (SPMT + marine equipment)", "MOBILIZATION", "Mammoet", -3, 1, "SPMT assembly + marine equipment mobilization", "Not Started"), _
        Array("PREP-001", "1.1", "Beam replacement + deck prep (D-ring, steel sets, welding)", "DECK_PREP", "Mammoet", -2, 2, "One-time setup for entire campaign", "Not Started"), _
        _
        Array("V1", "2.0", "TRIP 1: TR1 transport (1 unit)", "MILESTONE", "All", 0, 0, "Start Trip 1", "Not Started"), _
        Array("LO-101", "2.1", "TR1 load-out (RoRo) + stool-down", "LOADOUT", "Mammoet", 0, 1, "High tide required", "Not Started"), _
        Array("SF-102", "2.2", "TR1 sea fastening + lashing", "SEAFAST", "Mammoet", 1, 1, "Sea fastening / lashing", "Not Started"), _
        Array("SAIL-103", "2.3", "Trip 1 sail: MZP -> AGI (loaded)", "SAIL", "LCT Bushra", 2, 1, "Weather dependent", "Not Started"), _
        Array("ARR-104", "2.4", "AGI load-in TR1 + store on jetty", "AGI_UNLOAD", "Mammoet", 3, 1, "RoRo load-in", "Not Started"), _
        Array("RET-105", "2.5", "Trip 1 return: AGI -> MZP (empty)", "RETURN", "LCT Bushra", 4, 1, "Turnaround for Trip 2", "Not Started"), _
        _
        Array("V2", "3.0", "TRIP 2: TR2+TR3 transport (2 units) + Batch JD #1 (TR1-TR3)", "MILESTONE", "All", 4, 0, "Start Trip 2", "Not Started"), _
        Array("LO-201", "3.1", "TR2+TR3 load-out (RoRo) + sea fastening sequence", "LOADOUT", "Mammoet", 4, 3, "Two units sequential on LCT deck", "Not Started"), _
        Array("SAIL-202", "3.2", "Trip 2 sail: MZP -> AGI (loaded)", "SAIL", "LCT Bushra", 7, 1, "Check Shamal risk window", "Not Started"), _
        Array("ARR-203", "3.3", "AGI load-in TR3 + store on jetty", "AGI_UNLOAD", "Mammoet", 8, 1, "Unload sequence: TR3 first", "Not Started"), _
        Array("ARR-204", "3.4", "AGI load-in TR2 + store on jetty", "AGI_UNLOAD", "Mammoet", 9, 1, "Unload TR2", "Not Started"), _
        Array("JD1", "3.5", "BATCH JD #1: install TR1-TR3 (3 units)", "MILESTONE", "All", 10, 0, "Start installation when 3 units available", "Not Started"), _
        Array("STG-205", "3.6", "Batch #1 staging + steel bridge setup (Jetty->Bay route)", "TURNING", "Mammoet", 10, 1, "Crane/steel bridge; SPMT ready", "Not Started"), _
        Array("TURN-TR1", "3.7", "TR1 turning at bay", "TURNING", "Mammoet", 11, 3, "Includes transport to bay / precision turning", "Not Started"), _
        Array("JD-TR1", "3.8", "TR1 jack-down on temporary support", "JACKDOWN", "Mammoet", 14, 1, "Milestone: TR1 installed", "Not Started"), _
        Array("TURN-TR2", "3.9", "TR2 turning at bay", "TURNING", "Mammoet", 15, 3, "Includes transport to bay / precision turning", "Not Started"), _
        Array("JD-TR2", "3.10", "TR2 jack-down on temporary support", "JACKDOWN", "Mammoet", 18, 1, "Milestone: TR2 installed", "Not Started"), _
        Array("TURN-TR3", "3.11", "TR3 turning at bay", "TURNING", "Mammoet", 19, 3, "Includes transport to bay / precision turning", "Not Started"), _
        Array("JD-TR3", "3.12", "TR3 jack-down on temporary support", "JACKDOWN", "Mammoet", 22, 1, "Milestone: TR3 installed", "Not Started"), _
        Array("RET-TRIP2", "3.13", "Trip 2 return: AGI -> MZP (after Batch #1)", "RETURN", "LCT Bushra", 23, 1, "Reset for Trip 3", "Not Started"), _
        _
        Array("V3", "4.0", "TRIP 3: TR4+TR5 transport (2 units)", "MILESTONE", "All", 24, 0, "Start Trip 3", "Not Started"), _
        Array("LO-301", "4.1", "TR4+TR5 load-out (RoRo) + sea fastening sequence", "LOADOUT", "Mammoet", 24, 3, "Two units sequential on LCT deck", "Not Started"), _
        Array("SAIL-302", "4.2", "Trip 3 sail: MZP -> AGI (loaded)", "SAIL", "LCT Bushra", 27, 1, "Weather dependent", "Not Started"), _
        Array("ARR-303", "4.3", "AGI load-in TR5 + store on jetty", "AGI_UNLOAD", "Mammoet", 28, 1, "Unload sequence: TR5 first", "Not Started"), _
        Array("ARR-304", "4.4", "AGI load-in TR4 + store on jetty", "AGI_UNLOAD", "Mammoet", 29, 1, "Unload TR4", "Not Started"), _
        Array("RET-305", "4.5", "Trip 3 return: AGI -> MZP (empty)", "RETURN", "LCT Bushra", 30, 1, "Turnaround for Trip 4", "Not Started"), _
        _
        Array("V4", "5.0", "TRIP 4: TR6 transport (1 unit) + Batch JD #2 (TR4-TR6)", "MILESTONE", "All", 31, 0, "Start Trip 4", "Not Started"), _
        Array("LO-401", "5.1", "TR6 load-out (RoRo) + sea fastening", "LOADOUT", "Mammoet", 31, 2, "One unit; tide window", "Not Started"), _
        Array("SAIL-402", "5.2", "Trip 4 sail: MZP -> AGI (loaded)", "SAIL", "LCT Bushra", 33, 1, "Weather dependent", "Not Started"), _
        Array("ARR-403", "5.3", "AGI load-in TR6 + store on jetty", "AGI_UNLOAD", "Mammoet", 34, 1, "Unload TR6", "Not Started"), _
        Array("JD2", "5.4", "BATCH JD #2: install TR4-TR6 (3 units)", "MILESTONE", "All", 35, 0, "Start installation when 3 units available", "Not Started"), _
        Array("STG-404", "5.5", "Batch #2 staging + steel bridge setup (Jetty->Bay route)", "TURNING", "Mammoet", 35, 1, "Crane/steel bridge; SPMT ready", "Not Started"), _
        Array("TURN-TR4", "5.6", "TR4 turning at bay", "TURNING", "Mammoet", 36, 3, "Includes transport to bay / precision turning", "Not Started"), _
        Array("JD-TR4", "5.7", "TR4 jack-down on temporary support", "JACKDOWN", "Mammoet", 39, 1, "Milestone: TR4 installed", "Not Started"), _
        Array("TURN-TR5", "5.8", "TR5 turning at bay", "TURNING", "Mammoet", 40, 3, "Includes transport to bay / precision turning", "Not Started"), _
        Array("JD-TR5", "5.9", "TR5 jack-down on temporary support", "JACKDOWN", "Mammoet", 43, 1, "Milestone: TR5 installed", "Not Started"), _
        Array("TURN-TR6", "5.10", "TR6 turning at bay", "TURNING", "Mammoet", 44, 3, "Includes transport to bay / precision turning", "Not Started"), _
        Array("JD-TR6", "5.11", "TR6 jack-down on temporary support", "JACKDOWN", "Mammoet", 47, 1, "Milestone: TR6 installed", "Not Started"), _
        Array("RET-TRIP4", "5.12", "Trip 4 return: AGI -> MZP (after Batch #2)", "RETURN", "LCT Bushra", 48, 1, "Final return", "Not Started"), _
        Array("BUF-END", "Z.1", "Campaign buffer - weather / tide / port congestion reserve", "BUFFER", "All", 49, 2, "Reserve days to protect Feb completion target", "Not Started"), _
        Array("COMP", "99.0", "PROJECT COMPLETE - TR1-TR6 installed", "MILESTONE", "All", 50, 0, "Target: before 2026-03-01", "Not Started") _
    )
    ...
    For i = LBound(T) To UBound(T)
        wsData.Cells(r, COL_ID).Value = T(i)(0)
        wsData.Cells(r, COL_WBS).Value = T(i)(1)
        wsData.Cells(r, COL_TASK).Value = T(i)(2)
        wsData.Cells(r, COL_PHASE).Value = T(i)(3)
        wsData.Cells(r, COL_OWNER).Value = T(i)(4)
        wsData.Cells(r, COL_OFFSET).Value = CDbl(T(i)(5))
        wsData.Cells(r, COL_DUR).Value = CDbl(T(i)(6))
        wsData.Cells(r, COL_NOTES).Value = T(i)(7)
        wsData.Cells(r, COL_STATUS).Value = T(i)(8)

        s = d0Date + CDbl(T(i)(5))
        e = CalcEndDate(s, CDbl(T(i)(6)))
        wsData.Cells(r, COL_START).Value = s
        wsData.Cells(r, COL_END).Value = e
    ...
    MsgBox "Scenario 1-2-2-1 / JD (3 units x2) loaded into Schedule_Data." & vbCrLf & _
...
Public Sub RefreshGanttChart()
    ' Draws Gantt bars on Gantt_Chart based on Start/End.
...
Private Sub EnsureSheetsAndLayout()
    ' Creates missing sheets and minimal headers.
