#!/usr/bin/env python3
"""
JSON normalization script (robust version)
- Convert double quotes (\"\"\") to single quotes (\")
- Handle empty values: \"\"\"\" -> \"\"
- Merge string values split across lines
...
def normalize_double_quotes_robust(content: str) -> str:
    """
    Convert double quotes to single quotes (robust version)
    Handle empty values: \"\"\"\" -> \"\"
    """
    # 1. 빈 값 처리: """" -> "" (4개의 인용부호를 2개로)
    # 패턴: ": """" -> ": ""
    # 모든 경우를 처리: """" -> ""
    # 먼저 키-값 쌍의 빈 값을 처리
    content = re.sub(r'(":\s*)""""', r'\1""', content)
    
    # 2. 남은 4개의 연속된 인용부호를 2개로 변환
    while '""""' in content:
...
def fix_control_characters(content: str) -> str:
    """
    Remove control characters not allowed in JSON
    """
    # 문자열 값 안의 제어 문자 처리
    result = []
    in_string = False
...
def fix_unclosed_values(content: str) -> str:
    """
    Fix unclosed string values and malformed values
    Pattern: "key": " -> "key": ""
    Pattern: "key": "," -> "key": "",
    """
    # 1. 패턴: ": ", -> ": "",
    # 값이 쉼표로 잘못 들어간 경우 (가장 먼저 처리)
    content = re.sub(r'(":\s*"),(\s*\n)', r'\1"",\2', content)
    
    # 2. 패턴: ": "\s*\n\s*}, 또는 ": "\s*\n\s*{
    # 값이 닫히지 않고 다음 객체나 닫는 중괄호가 오는 경우
    content = re.sub(r'(":\s*")(\s*\n\s*[},])', r'\1"\2', content)
    
    # 3. 패턴: ": "\s*\n\s*"key" -> ": ""\n    "key"
    # 값이 닫히지 않고 다음 키가 오는 경우
    content = re.sub(r'(":\s*")(\s*\n\s*"[^"]*":)', r'\1"\2', content)
    
    # 4. 패턴: ": "value"other" -> ": "valueother"
    # 값 안에 잘못된 인용부호가 있는 경우
    content = re.sub(r'(":\s*")([^"]*)"([^",}\n]*)"', r'\1\2\3', content)
...
def normalize_json_content(content: str) -> str:
    ...
    # 2. 이중 인용부호 정규화 (빈 값 처리 포함)
    content = normalize_double_quotes_robust(content)
    
    # 3. 닫히지 않은 값 수정
    content = fix_unclosed_values(content)
    
    # 4. 줄바꿈이 값 안에 포함된 케이스 처리
    ...
    # 6. 남은 이중 인용부호 제거 (혹시 모를 경우)
    content = content.replace('""', '"')
    
    # 7. 다시 한 번 닫히지 않은 값 수정 (제어 문자 처리 후)
    content = fix_unclosed_values(content)
