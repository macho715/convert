            ...
            if end_d is not None:
                ...
                for j in range(i + 1, min(i + 10, len(lines))):
                    if "THU/FRI" in lines[j].upper() and "WAVE" in lines[j].upper():
                        ...
                        if w is not None:
                            by_date[fri] = {"wave_ft": w, "wind_kt": _parse_wind_kt(lines[j])}
                    if "FRI/SAT" in lines[j].upper() and "WAVE" in lines[j].upper():
                        w = _parse_wave_ft(lines[j])
                        if w is not None:
                            by_date[end_d] = {"wave_ft": w, "wind_kt": _parse_wind_kt(lines[j])}
                i += 1
                continue
...
def build_weather_records(
    ...
    while d <= end_date:
        ...
        records.append({
            ...
            "source": "PDF_PARSED",
            "notes": f"Parsed from ADNOC PDF; wave {wave_ft} ft" if wave_ft is not None else "Parsed from PDF",
            "risk_level": risk,
            "is_shamal": False,
        })
        d += timedelta(days=1)
...
def run(
    ...
    for d in [start_date + timedelta(days=k) for k in range((end_date - start_date).days + 1)]:
        prev = d - timedelta(days=1)
        if prev in by_date and by_date[prev].get("wave_ft") == 6.0 and by_date.get(d, {}).get("wave_ft") == 5.0:
            if d not in by_date:
                by_date[d] = {}
    ...
    payload = {
        ...
        "location": {"name": "Mina Zayed Port / AGI Site", "latitude": 24.12, "longitude": 52.53},
    ...
    if out_path is None:
        ...
        out_path = Path(out_path) if not Path(out_path).is_absolute() else Path(out_path)
...
def main() -> int:
    ...
    for i, a in enumerate(sys.argv[1:], 1):
        if a == "--out" and i < len(sys.argv):
            out_path = sys.argv[i]
            break
    parsed_dir = Path(args[0]) if args else (CONVERT_ROOT / "out" / "weather_parsed" / "20260128")
    today = date.today()
    start_date = today
