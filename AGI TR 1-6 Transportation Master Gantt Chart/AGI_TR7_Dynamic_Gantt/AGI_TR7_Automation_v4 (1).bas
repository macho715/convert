...
Private Sub FetchDailyCsvWindKn(ByVal lat As Double, ByVal lon As Double, ByVal tz As String, ByVal horizon As Long, ByRef dates() As String, ByRef values() As Double)
    ...
    url = "https://api.open-meteo.com/v1/forecast" & _
          ...
          "&daily=windspeed_10m_max" & _
          "&timezone=" & UrlEncode(tz) & _
          "&forecast_days=" & CStr(horizon) & _
          "&format=csv"
    Dim csv As String
    csv = HttpGet(url)
    ParseDailyCsv csv, "windspeed_10m_max", dates, values
...
Private Sub ParseDailyCsv(ByVal csv As String, ByVal seriesName As String, ByRef dates() As String, ByRef values() As Double)
    ' Parses Open-Meteo CSV where first column is date and one column matches seriesName.
    Dim lines() As String
    lines = Split(csv, vbLf)
    If UBound(lines) < 2 Then Exit Sub

    Dim hdr() As String
    hdr = Split(Trim(lines(0)), ",")
    Dim idx As Long
    ...
    For i = 1 To UBound(lines)
        If Len(Trim(lines(i))) > 0 Then n = n + 1
    Next i
    ...
    For i = 1 To UBound(lines)
        Dim ln As String
        ln = Trim(lines(i))
        If Len(ln) = 0 Then GoTo NextI
        Dim parts() As String
        parts = Split(ln, ",")
        dates(r) = parts(0)
        If idx <= UBound(parts) Then
            values(r) = Val(parts(idx))
        Else
            values(r) = 0
        End If
        r = r + 1
