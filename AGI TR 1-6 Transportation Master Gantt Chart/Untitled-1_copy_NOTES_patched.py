...
def load_tasks_from_tsv(tsv_path, project_start_date):
    ...
    with open(tsv_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            task_id_candidate = (
                row.get("Activity ID", "").strip() or row.get("ID", "").strip()
            )
            task_name_raw = (
            ...
            if (
                task_name_raw
                and _is_number_like(task_name_raw)
                and task_id_candidate
                and not _is_number_like(task_id_candidate)
            ):
            ...
            if not task_id:
                if "VOYAGE" in task_name_raw.upper():
                    ...
                    "DECK" in task_name_raw.upper() and "PREP" in task_name_raw.upper()
                ):
                    task_id = "PREP-001"
                else:
                    task_counter += 1
            ...
            if phase_raw:
                phase = phase_mapping.get(
                    phase_raw, infer_phase_from_task(task_name_raw)
                )
            else:
