...
### 1. ApplyStatusFormatting (상태별 색상 적용)
- **기능**: 상태 값에 따라 자동으로 색상 서식 적용
- **실행**: `Alt + F8` → `ApplyStatusFormatting` 선택 → 실행
- **색상 규칙**:
  - 🟢 Complete: 연두색
  - 🟡 In Progress: 연노랑
  - 🔴 Not Started: 연빨강
...
Private Sub Workbook_Open()
    Application.OnKey "^+R", "AutoRefreshAll"      ' Ctrl+Shift+R
    Application.OnKey "^+S", "QuickStatusUpdate"   ' Ctrl+Shift+S
    Application.OnKey "^+P", "ExportToPDF"         ' Ctrl+Shift+P
End Sub
```
