...
1. **자동 설치**: `ONE_CLICK_INSTALL.bat` 실행 → `AGI_TR6_VBA_Enhanced_AUTOMATION_READY.xlsm` 생성
2. **워크북 열기**: 생성된 `.xlsm` 파일 열기 → "콘텐츠 사용(Enable Content)" 클릭
3. **실행**: `Control_Panel` 시트 버튼으로 실행
...
| `ONE_CLICK_INSTALL.bat` / `.vbs` | VBA 자동 주입 + XLSM 생성 (권장) |
...
```
실행 후 `AGI_TR6_VBA_Enhanced_AUTOMATION_READY.xlsm` 파일이 생성됩니다.
...
### Control_Panel 버튼 설정
`Control_Panel` 시트에서 버튼(도형) 만들고 아래 매크로 연결:
- `RefreshAll` / `UpdateAllDates` / `GenerateDailyBriefing`
- `ExportToCSV` / `BackupWorkbook` / `ExportVisibleSheetsToPDF`
- `RunPythonUpdate` / `ValidateScheduleData`
...
**모드 옵션:**
- `--mode update`: 스케줄 업데이트 및 간트 차트 재생성
- `--mode validate`: 스케줄 데이터 검증만 수행
...
### VBA 기능
- `UpdateAllDates`: D0 기반 일정 자동 업데이트 (단축키: `Ctrl+Shift+U`)
...
- `ValidateScheduleData`: 스케줄 데이터 검증
...
`SETTINGS` 시트에서 다음 값 설정:

| 설정 항목 | 설명 | 예시 |
| --- | --- | --- |
| `PYTHON_EXE` | Python 실행 파일 경로 | `py` 또는 `C:\Python311\python.exe` |
| `PY_SCRIPT` | agi_tr_runner.py 절대경로 | `C:\Users\...\agi_tr_runner.py` |
| `OUT_DIR` | 결과 저장 폴더 | `C:\Temp\AGI_TR_Output` |
| `BACKUP_DIR` | 백업 폴더 | `C:\Temp\AGI_TR_Backup` |
| `LOG_FILE` | 로그 파일 경로 | `C:\Temp\tr6_ops.log` |
